...
function sanitizeForPpt(text) {
    ...
    for (const character of text) {
        ...
        const isValidXml = codePoint === 0x9 ||
            codePoint === 0xa ||
            codePoint === 0xd ||
...
function htmlToPptRuns(html) {
    ...
    function pushRun(text, format, paraOpts) {
    ...
    function walk(node, format, state) {
...
async function toDataUrl(urlOrPath) {
    if (!urlOrPath) {
        return '';
    }
    if (/^https?:\/\//i.test(urlOrPath)) {
        ...
        const extension = node_path_1.default.extname(new URL(urlOrPath).pathname).toLowerCase();
        const mimeType = extension === '.jpg' || extension === '.jpeg' ? 'image/jpeg' : 'image/png';
        return `data:${mimeType};base64,${Buffer.from(arrayBuffer).toString('base64')}`;
    }
    const candidates = [
        node_path_1.default.resolve(process.cwd(), urlOrPath),
        node_path_1.default.resolve(process.cwd(), '..', urlOrPath)
    ];
    for (const candidate of candidates) {
        try {
            const contents = await (0, promises_1.readFile)(candidate);
...
function addSlideHeader(slide, title, theme, bgImageData) {
...
function addPipelineLegend(slide, pres, theme, chartX, chartW) {
    ...
    function addLegendItem(col, row, symbol, text, options) {
...
async function generateReportPpt(input) {
    ...
    if (topOfMindRuns.length) {
        topOfMindSlide.addText(topOfMindRuns, {
            x: PPT_X,
            y: TABLE_TOP_Y,
            w: PPT_W,
            h: 6.1,
    ...
    function renderPipelineSlides(title, pageGroups, presentation) {
    ...
    for (const slideData of slides) {
        ...
        for (let pageIndex = 0; pageIndex < pageSizes.length; pageIndex += 1) {
            ...
            pageSlide.addTable([headerRow, ...pageRows], {
                x: PPT_X,
                y: FEATURE_TABLE_TOP_Y,
                w: PPT_W,
