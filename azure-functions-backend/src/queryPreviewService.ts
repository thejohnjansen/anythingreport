...
export interface SlidesPreviewResult {
  slides: PreviewSlide[];
  linkBase: string;
  hasMidpoint: boolean;
...
function parseAreaLevel3(areaPath: string | undefined): string {
  const parts = String(areaPath || '').split('\\').filter(Boolean);
  return parts[2] || '';
}

function findFirstLeafEpic(queryResult: WiqlTreeResult, workItemMap: Record<number, AzureDevOpsWorkItem>): AzureDevOpsWorkItem | null {
...
function findLeafEpics(queryResult: WiqlTreeResult, workItemMap: Record<number, AzureDevOpsWorkItem>): AzureDevOpsWorkItem[] {
  const children: Record<number, number[]> = {};
  const orderedTargets: number[] = [];
  const seenTargets = new Set<number>();
  ...
  return orderedTargets
    ...
    .filter((workItem): workItem is AzureDevOpsWorkItem => {
      const workItemType = String(workItem?.fields?.['System.WorkItemType'] || '').toLowerCase();
      return !!workItem && workItemType.includes('epic') && (children[workItem.id] || []).length === 0;
    });
}

function buildTitleContext(queryResult: WiqlTreeResult, workItemMap: Record<number, AzureDevOpsWorkItem>) {
...
async function getFieldsAsOf(baseUrl: string, project: string, workItemId: number, asOfDate: string): Promise<MidpointFields> {
  ...
  let best: AzureDevOpsWorkItem | null = null;
...
async function batchHistoricalFields(
  baseUrl: string,
  project: string,
  ids: number[],
  asOfDate: string
...
function buildSlides(
  ...
  midpointMap: Record<number, MidpointFields> | null
): PreviewSlide[] {
  const children: Record<number, number[]> = {};
  const targetIds = new Set<number>();
  const rootIds = new Set<number>();

  function typeContains(workItem: AzureDevOpsWorkItem | undefined, needle: string): boolean {
    const workItemType = String(workItem?.fields?.['System.WorkItemType'] || '').toLowerCase();
  ...
  const additionalItems: SlideItem[] = [];
  const seenAdditional = new Set<number>();
  ...
  const candidateEpicIds = new Set<number>();
  ...
  for (const epicId of candidateEpicIds) {
    ...
    const rows = (children[epicId] || []).map((id) => workItemMap[id]).filter(Boolean) as AzureDevOpsWorkItem[];
...
function buildFlatSlides(
  ...
  workItemMap: Record<number, AzureDevOpsWorkItem>,
  midpointMap: Record<number, MidpointFields> | null,
  flatSlideTitle: string,
  flatSlidePrefix: string
): PreviewSlide[] {
  const children: Record<number, number[]> = {};
  const targetIds = new Set<number>();
  ...
  const itemsByArea: Record<string, SlideItem[]> = {};
  const orderedAreas: string[] = [];

  function pushItem(areaName: string, item: SlideItem): void {
